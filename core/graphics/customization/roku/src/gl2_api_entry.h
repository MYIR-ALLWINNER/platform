#ifndef _GL2_API_ENTRY_H
#define _GL2_API_ENTRY_H

extern __thread gpu_surface_t *g_gpu2disp;
...
void API_ENTRY(glDrawArrays)(GLenum mode, GLint first, GLsizei count) {
	ATRACE_CALL()
#if 0
	if (g_gpu2disp != NULL && g_gpu2disp->use_fbo) {
		printf("aw: glDrawArrays, glBindFramebuffer, current_fbo:%d, fbo:%d\n", g_gpu2disp->current_fbo,
			g_gpu2disp->fbo[g_gpu2disp->current_fbo]);
		gpu_con.gl2.glBindFramebuffer(GL_FRAMEBUFFER, g_gpu2disp->fbo[g_gpu2disp->current_fbo]);
	} else {
		printf("aw: glDrawArrays render to native surface\n");
		gpu_con.gl2.glBindFramebuffer(GL_FRAMEBUFFER, 0);
	}
#endif
