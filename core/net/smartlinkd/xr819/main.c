...
int main(int argc, char** argv){
    struct _cmd send_server_data;
    struct _info *net_info = &send_server_data.info;
	int ret;
		...
		/*device is get ssid and password, now need to connect ap*/
			...
			if (while_test == 0) {
				...
				if(strcmp(server_state,"yes")==0){
					strncpy(net_info->base_info.ssid,complete->ssid,complete->ssid_size+1);
					strncpy(net_info->base_info.password,complete->pwd,complete->pwd_size+1);
					*(net_info->base_info.ssid+complete->ssid_size)='\0';
					*(net_info->base_info.password+complete->pwd_size)='\0';

					if(complete->round_num % 2 == 0)
						net_info->base_info.security=2;
					else
						net_info->base_info.security=1;

					net_info->protocol = AW_SMARTLINKD_PROTO_XRSC;
					aw_smartlinkd_init(0,xr_server_thread_handle);
					aw_easysetupfinish(&send_server_data) != sizeof(struct _cmd);
