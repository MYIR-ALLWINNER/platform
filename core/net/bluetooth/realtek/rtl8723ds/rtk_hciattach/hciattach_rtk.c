...
#define RTK_VERSION "3.1.95f7bb4.20180821-190218"
...
{
	if (offset == 0x0044) {
		if (chip_type > CHIP_BEFORE)
			return 1;
		/* special for 8723CS */
		if (chip_type == CHIP_8723CS_XX ||
		    chip_type == CHIP_8723CS_CG ||
		    chip_type == CHIP_8723CS_VF)
			return 1;
	}
			       ...
			       uint8_t bt_addr[6], uint32_t *result)
{
	...
	if (!(flags & CONFIG_BTMAC) && customer_bdaddr) {
		...
		if (pent->chip_type == CHIP_8723CS_XX ||
		    pent->chip_type == CHIP_8723CS_CG ||
		    pent->chip_type == CHIP_8723CS_VF) {
			b[0] = 0x44;
			b[1] = 0x00;
		}
