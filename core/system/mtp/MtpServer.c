...
static MtpResponseCode doSendPartialObject(struct MtpServer *mServer)
{
	...
	DLOG("receiving partial %s offset:%llu length:%u", edit->mPath, offset, length);
...
static MtpResponseCode doGetDeviceInfo(struct MtpServer *mServer)
{
	...
	/* Model */
	mData->putString("kunos", mData);
	/* Device Version */
	mData->putString("1.0", mData);
	/* Serial Number */
	mData->putString("20080411", mData);
