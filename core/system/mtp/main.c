...
static mtp_storage_t gStorageArray[] = {
	{NULL, 65537, "/mnt/UDISK", "KunOS存储设备", 0, 0},
...
static int kunos_usb_uevent_socket(void)
{
...
static int kunos_usb_uevent_detect(int socket)
{
...
static void usb_uevent_handle(int arg)
{
	int socket = arg;
	int ret = -1;
	ret = kunos_usb_uevent_detect(socket);
	switch(ret){
...
int main()
{
	...
	/* open netlink socket */
	socket = kunos_usb_uevent_socket();
	if (socket < 0)
		return -1;
...
int main()
{
	...
	//mStorage = MtpStorageInit(65537, "/mnt/UDISK", "KunOS存储设备", 0, MB(1));
	mStorage = MtpStorageInit(65537, "/mnt/UDISK", "KunOS存储设备", 0, 0);
