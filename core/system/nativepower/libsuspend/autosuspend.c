...
#ifdef USE_SUSPEND
	autosuspend_ops = autosuspend_suspend_init();
	if (autosuspend_ops) {
		goto out;
	}
#else
...
/* Remove autosleep so userspace can manager suspend/resume and keep stats */
	...
	if (!autosuspend_ops) {
		printf("failed to initialize autosuspend\n");
		return -1;
	}
...
out:
	...
	printf("autosuspend initialized\n");
	return 0;
}
...
{
	if (wakeup_func != NULL) {
		printf("Duplicate wakeup callback applied, keeping original");
		return;
	}
	wakeup_func = func;
}
