...
static void *suspend_thread_func(void *arg __attribute__ ((unused)))
{
	...
	while (1) {
		...
		success = true;
		printf("%s: write %*s to wakeup_count\n", __func__, wakeup_count_len, wakeup_count);
...
static int autosuspend_wakeup_count_enable(void)
{
	...
	printf("autosuspend_wakeup_count_enable\n");

	ret = sem_post(&suspend_lockout);

	if (ret < 0) {
		strerror_r(errno, buf, sizeof(buf));
		printf("Error changing semaphore: %s\n", buf);
	}

	printf("autosuspend_wakeup_count_enable done\n");
...
static int autosuspend_wakeup_count_disable(void)
{
	...
	printf("autosuspend_wakeup_count_disable\n");
	...
	printf("autosuspend_wakeup_count_disable done\n");
