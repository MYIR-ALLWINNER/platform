...
void parent(const char *tag, int parent_read)
{
	...
	if (wait(&status) != -1) {  /* Wait for child */
		if (WIFEXITED(status)) {
			KTAGLOGD(tag, "terminated by exit(%d)",
					WEXITSTATUS(status));
		} else if (WIFSIGNALED(status)) {
			KTAGLOGD(tag, "terminated by signal %d",
					WTERMSIG(status));
		} else if (WIFSTOPPED(status)) {
			KTAGLOGD(tag, "stopped by signal %d",
					WSTOPSIG(status));
		}
	} else {
		KTAGLOGE(tag, "wait() failed: %s (%d)", strerror(errno), errno);
	}
...
void child(int argc, char *argv[])
{
	...
	if (execvp(argv_child[0], argv_child)) {
		KTAGLOGE(tag, "executing %s failed: %s\n",
